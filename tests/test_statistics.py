...
def test_statistics_alphabetical_sorting(project_with_sources):
    ...
    expected_order = ["ID", "mesh_area", "mesh_volume", "water_tight"] # alphabetical order
...
def test_statistics_summary_dataframe(project_with_sources):
    ...
    assert pd.isna(std_row["water_tight"]) # Standard deviation should be NaN for boolean columns
...
def test_statistics_integration(project_with_sources):
    ...
    for prop in stats.get_properties():
        if prop in stats.get_contact_properties(): # if dynamic property
            ...
            assert stats_df[dynamic_cols[0]].notna().any(), f"Data for {prop} is entirely NaN."
        else: # if static property
            assert prop in stats_df.columns, f"Static property {prop} is missing from DataFrame."
